...
app.post('/api/livekit/token', async (req, res) => {
  try {
    ...
    // Create access token
    console.log('Creating AccessToken with:', { apiKey, apiSecret, participantName });
    ...
    try {
      ...
      console.log('JWT generated:', typeof jwt, jwt);
      
      if (typeof jwt !== 'string') {
        console.error('JWT is not a string after awaiting:', typeof jwt);
        return res.status(500).json({ error: 'JWT generation returned non-string value' });
      }
    } catch (error) {
      console.error('Error generating JWT:', error);
      return res.status(500).json({ error: 'Failed to generate JWT: ' + error.message });
    }
...
// Serve the real LiveKit client interface
app.get('/real-livekit', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'real-livekit-client.html'));
});
...
server.listen(PORT, () => {
  ...
  console.log(`🔗 Real LiveKit Client: http://localhost:${PORT}/real-livekit`);
  console.log(`🔗 Simulated Client: http://localhost:${PORT}/livekit`);
